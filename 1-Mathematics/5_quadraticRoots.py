...
def quadraticRoots(a,b,c):
    ...
    temp=pow(b,2)-4*a*c #value of b^2-4ac
    ...
    if temp<0:#if b^2-4ac is less then zero then roots are imaginary
        ...
        root1=math.floor((-1*b+math.sqrt(temp))/(2*a)) #root1

        root2=math.floor((-1*b-math.sqrt(temp))/(2*a))#root2

        print(root1,root2) #printing the answer